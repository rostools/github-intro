...
Before we start the more practical part of the workshop, we'll take some time to refresh your
...
getting you to do a discussion activity, then getting
...
repeated exposures to it, by both getting you to retrieve, discuss, and reread it. Git requires you work with files in a
fundamentally different way than people usually work, so we want to give you multiple of exposures to Git
does before moving into the practical parts of this workshop. By including this repetition we aim to
reinforce these new concepts.
:::
...
-   **Top repositories** list and search bar at the top of the left
    ...
    only a few) repositories listed here. When you start to work with more repositories, this is a quick way to access them.
...
-   **Issues** button on the navigation bar on the top right that looks
    ...
    experience, we don't use this item often, but can be useful to
...
some commonly used areas of GitHub, let's create a
new repository on GitHub. This is where we will store all the files
and folders for the project we will be working on in this workshop.
...
`README.md` file is a common file included in repositories to
describe what the repository contains and is used for.
...
We will start to add and modify files and in the repository in the next
...
-   `.`: One dot means the current folder. If you're in the folder
    ...
    folder within the `username` folder, like so: `/Users/username/Documents/`.
...
Before we get into hands-on work with files on GitHub in the next session, we'll briefly do
a walkthrough of what happens when you add or modify files stored in a
Git repository. In @fig-git-commits, we have a
hypothetical series of commits (i.e., actions or changes) done in a Git repository. Each commit is shown as a "circle" or "dot" on a line that forms the Git history. The messages beside each commit is the commit message describing what the commit is about.

As we make changes to files or folders in the repository another commit is created. 

The name "main" in the diagram is the default name of the "branch" that contains the saved
...
5.  Add a new recipe file: 'baked-goods/cookies.md'
    ...
    ├── README.md
    ├── soup/tomato.md
    └── baked-goods/cake.md <- new file in new folder
    ```
...
2.  Pair up with your neighbour and for the next 3 minutes, take turns
    ...
    about the basics of Git and GitHub. Try to come to a shared understanding of
...
-   GitHub's landing page as well as it's navigation bar has several
    quick links to commonly used areas of GitHub (such as the "Create new" button, notifications inbox and issues)
...
-   Each change to a file in a repository creates a new commit in the
    ...
    GitHub).
-   Commits are connected to each other creating the history of
    changes made in within a repository.
