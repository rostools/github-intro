...
You can verbally relate `.md` to `.qmd`, but not get into it too much.
:::
...
Reinforce that paths are pointers to files on your computer, that they
are for us humans to effectively organise and work with files. That
every file has a parent folder, and every folder may also have a parent
folder. That files and folders are separated by `/` or `\` and that the
last item in the path is either a file or a folder. That a path is like
a family tree, where each parent can have one or more child, but each
child only has two sets of parents.
:::
...
have the changes, we're able to see exactly what was changed. You can
even go back to the state of the folder, with the exact files and their
