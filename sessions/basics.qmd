...
-   GitHub requires that whenever you edit a file, 
    you need to commit those changes *right away*. When using Git on your computer,
...
GitHub offers many tools to help you manage your project if its
files are stored there. For instance, GitHub has "Issues"
that allow you to create tasks or notes related to the project. We'll create our own GitHub Issues later in this workshop.
...
file-level changes. This means you can see the specific changes made in a
file, not just that it was changed. OneDrive and Dropbox also use a
...
-   **Top repositories** list and search bar at the top of the left sidebar.
   Since you are new to GitHub, you likely won't have any (or only
...
-   **Notifications inbox** button on navigation bar on the top right
    ...
    repositories, like when someone comments on an issue you made or mentions you in an issue.

We'll return to these items later in the workshop, but for now,
it's enough for you to know where they are. Let’s go to the next section and create a new repository :rocket:
...
Now that we've gone over the landing page of GitHub and taken a look at some
...
that are currently in this repository. We will go over adding
...
Why is this important or useful? Because we humans have
...
-   Each change to a file in a repository creates a new commit in the
    Git history log, each with their own commit message (when working on GitHub).
