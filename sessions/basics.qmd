...
-   GitHub makes it required that whenever you edit a file, *right away*
    you need to commit those changes. When using Git on your computer,
    you don't need to commit changes right away.
...
GitHub adds a lot of tools to help you with working on a project if you
keep your project's files on GitHub. For instance, GitHub has "Issues"
that allow you to create tasks or notes related to the project.
...
%%| fig-cap: "How Git and GitHub work together by synchronising changes between GitHub and your computer."
...
Dropbox, GitHub is tracks line-level changes to files, not just
file-level changes. This means you can see exactly what changed in a
file, not just that a file changed. OneDrive and Dropbox also use a
...
team or group. But they aren't so good for collaboratively working
...
-   **Top repositories** list and search bar on the left side bar at the
    top. Since you are new to GitHub, you likely won't have any (or only
    a few) repositories listed here.
...
-   **Notifications inbox** button on navigation bar on the top right
    ...
    repositories, like when someone comments on an issue you made.

We'll return to some of these items later in the workshop, but for now,
let's go to the next section to create a new repository.
...
Now that we've gone over the landing page of GitHub and gone to some
commonly used areas of GitHub, let's use GitHub for it's main purpose of
working with and managing files. We'll do that by creating a new
...
so it is in one place and organised. Or if you can have a repository to
store administration files for your lab or group.

For this workshop, we'll create a repository to keep files with recipes
:yum: :cake: :pizza: :stew:
...
| Repository name | This is the name of the repository. | Name it `recipes`. |
| Description | This is a short description of what the repository is for and what it will contain. | Write "Keeping our favourite recipes". |
...
| Add a README file | This is a file that is shown on the front page of the repository. It is a good place to put information about the repository. | Make sure it is checked. |
| Add .gitignore | This is a file that tells Git to ignore certain files or folders. | Leave it unchecked. |
| Add a license | This is a file that tells others how they can use the files in the repository. | Leave it unchecked. |
...
that you will eventually have in this repository. We will go over adding
and modifying files in the repository in the next section. For now, this
is the perfect time to talk about something that is fundamental to
working with files and folders: file paths.
...
You may have noticed that when we created the repository, a file called
`README.md` was automatically created. The `.md` file extension stands
for [Markdown](https://quarto.org/docs/authoring/markdown-basics.html)
and is a plain text file format (a type of file, like Word's `.docx`)
that's designed to be easy to write and read.
...
## An visual walkthrough of working with files on GitHub
...
hypothetical series of actions done in a Git repository on GitHub that create
commits that are added to the Git history log each time.

When we first create a repository, at least when created on GitHub,
often it will automatically add a file called `README.md` which creates
the first commit of the repository. The `README.md` file is a common
file included in repositories as describes what the repository contains
and is used for. The next step is to add the first file (aside from the
`README.md`) to the repository, which again creates a new commit. As we
make a change to that file, this creates another commit. With each new
file added or when an existing file is modified, moved, deleted, or
renamed, a new commit is created. As shown in @fig-git-commits, each
step made adds a new commit, which is the "circle" or "dot" on the
diagram, to the Git history log.

```{mermaid}
%%| label: fig-git-commits
%%| fig-cap: "Each step creates a new commit in the history log of the Git repository on GitHub."
%%| fig-alt: "A diagram showing a series of commits in a Git repository, with text that is expanded on below the diagram."
%%{init: {'themeVariables': {
  'git0': '#ba8b00', 
...
gitGraph TB:
  checkout main
  commit id: "1. Created repo with README: 'README.md'"
  commit id: "2. Add a new recipe file: 'tomato-soup.md'"
  commit id: "3. Fix a typo: 'tomato-soup.md'"
  commit id: "4. Move file to new folder: 'soups/tomato.md'"
  commit id: "5. Add a new recipe file: 'baked-goods/cake.md'"
```

When we look at the files that exist in the repository with each new
commit, they look like:

1.  Created repo with README: 'README.md'
...
2.  Add a new recipe file: 'tomato-soup.md'
...
5.  Add a new recipe file: 'baked-goods/cake.md'
...
Because of this history of connected commits that each contain changes
made to a file, it allows you to be able to navigate this history and
see exactly what was changed. You can even go back to the state of the
folder, with the exact files and their contents that existed at that
point in time. Why is this important or useful? Because we humans have
imperfect memories, so having it recorded in a history log makes it much
easier to recall what you or your collaborators were doing at a specific
point in time as well as when a specific change was added and why.
...
-   Each change to a file in a repository creates a new commit in the
    Git history log (when working on GitHub).
