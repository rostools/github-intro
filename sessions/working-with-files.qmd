...
changes. {{< var github-commit-start >}} This will open a pop-up where
we can add a commit message to describe the changes.

You might have noticed that GitHub automatically creates a commit
message for you. It's good practice to edit it to be descriptive, as it
...
a descriptive message for these changes by writing "Start of soup recipe
md file".

You usually don't need to add an extended description of the changes, so
we can leave it empty. {{< var github-commit-end >}}
...
3.  In the new file, write one or two lines of text of some basic
    ...
    point is to write something here.
...
{{< var github-commit-start >}} Write the commit message for these
changes as "Rename soup.md to tomato-soup.md".
{{< var github-commit-end >}}
...
`soups/` and then move the `tomato-soup.md` file into it. Noticed that
...
typo or want to change it.

But we can still change it! If we make a mistake in the folder name, we
...
{{< var github-commit-start >}} Write a message for these changes to be
"Move tomato soup recipe into a new soups folder".
{{< var github-commit-end >}}

We've moved it! You can see the new folder `soups/` in the `Code` tab of
