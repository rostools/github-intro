...
changes by clicking the green "Commit changes" button in the top right
corner of the page.

As the commit message, we'll write "Add tomato soup recipe". We don't
need to write an extended description. We'll keep the "Commit directly
to the `main` branch" option selected, then click the "Commit changes"
button.
