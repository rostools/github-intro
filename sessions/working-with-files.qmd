...
-   **Use hyphens or underscores**: If you need to separate words in a
    ...
    introduce different issues in some situations and contexts. For
    example, different operating systems (Windows, Mac, Linux, etc.)
    handle spaces differently, so transferring files with spaces in
    their names between systems can lead to issues. So, it’s best to
...
1.  Click the pencil icon {{< var pencil-icon >}} on the top-right side
    of the page ("Edit this file") to edit the `cookie-recipe.md` file.
...
It is possible to both explore the history of the entire repository and
...
Now that we've explored how to add files and folders to a GitHub
repository, let's take a look at how to upload files to GitHub.
...
the page. In the dropdown menu, we'll click "Upload files".
This leads us to a new page where we can drag and drop files from our
computer or click the "choose your files" button to select files from
our computer.
...
## :technologist: Upload a file to your repository

**Time: ~10 minutes.**

Now, let's upload a file to your repository. You can either upload a random file from your compyter (a file that you don't mind sharing!) or you can download our [icon](https://raw.githubusercontent.com/rostools/github-intro/refs/heads/main/_extensions/rostools/r3-theme/favicon.ico) and upload that to your repository. Alternatively, you can find an image of a cookie or recipe book from a website like [Pixabay](https://pixabay.com/) and upload that to your repository.
...
2.  Drag and drop the file you want to upload into the upload area or
    ...
    computer.
3.  Once you've selected the file, add a commit message to
    describe the changes as you've done before. 
4.  Click the "Commit changes" button to upload the file to your
    repository.

::: callout-note

Unfortunately, the file will be uploaded to the root of the repository
and not in a folder. This is a limitation of GitHub's upload feature.

If you want to move the file to a folder, you can do so by going to the
file in the repository and clicking the pencil icon {{< var
pencil-icon >}} on the top-right side of the page. Then, you can
change the file path to the folder you want to move it to as we did previously with the recipe files.
:::
